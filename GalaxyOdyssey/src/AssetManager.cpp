...
SDL_Texture * AssetManager::GetTexture(std::string file_name){
    ...
    if (mTextures[fullpath] == nullptr) {
        ...
        if (mTextures[fullpath] == nullptr)
            std::cout << "[ERROR]: Texture loading Error: Texture: " << file_name.c_str() << "\nError: " << SDL_GetError() << '\n';
    }
...
TTF_Font * AssetManager::GetFont(std::string file_name, int size){
    ...
    if (mFonts[key] == nullptr) {
        ...
        if (mFonts[key] == nullptr)
            std::cout << "[ERROR]: Font Loading Error: Font: " << file_name.c_str() << "\nError: " << TTF_GetError() << '\n';
    }
    return mFonts[key];
}
...
SDL_Texture * AssetManager::GetText(std::string text, std::string file_name, int size, SDL_Color color){
    ...
    if (mText[key] == nullptr) {
        ...
        if (mText[key] == nullptr)
            std::cout << "[ERROR]: Font Texture creating Error: Font: " << file_name.c_str() << "\nError: " << SDL_GetError() << '\n';
    }
    return mText[key];
}
...
Mix_Music* AssetManager::GetMusic(std::string filename) {
	...
	if(mMusic[fullPath] == nullptr) {
		...
		if(mMusic[fullPath] == nullptr)
			printf("[ERROR]: Music Loading Error: File-%s Error-%s", filename.c_str(), Mix_GetError());
	}
...
Mix_Chunk* AssetManager::GetSFX(std::string filename) {
		...
		if(mSFX[fullPath] == nullptr) {
			...
			//Error handling for file loading
			if (mSFX[fullPath] == nullptr)
				printf("[ERROR]: SFX Loading Error: File-%s Error-%s", filename.c_str(), Mix_GetError());
		}
		//returning the cached file from the map
