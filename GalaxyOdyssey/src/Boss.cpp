...
Boss::Boss(int path){
    ...
    mBossTex = new AnimatedTexture("Boss.png", 0.0f, 0.0f, 400.0f, 400.0f, 28, 2.0f, AnimatedTexture::horizontal);
    ...
    switch(mCurrentAttack){
        case NORMAL:
            if(Active() && InPosition() && NoCollisions()){
                ...
                if(currentTime - mLastFiredNormal >= mNormalDelay){
                    for(int i = 0; i < MAX_BULLETS; i++){
                        if(!mBullet[i]->Active()){
                            Vector2 pos2 = Vector2(Pos().x + 140.0f, Pos().y + 80.0f);
                            mBullet[i]->FireBullet(pos2);
            ...
            if(Active() && InPosition() && NoCollisions()){
                ...
                if(currentTime - mLastFiredSeries >= mSeriesDelay){
                    for(int i = MAX_BULLETS; i < MAX_BULLETS + 3; i++){
                        if(!mBullet[i]->Active()){
                            Vector2 pos = Vector2(Pos().x - 145.0f, Pos().y + 80.0f);
        ...
        case SERIES_BOUNCING:
            if(Active() && InPosition() && NoCollisions()){
                Uint32 currentTime = SDL_GetTicks();
                if(currentTime - mLastFiredSeries >= mSeriesDelay){
                    for(int i = MAX_BULLETS*2; i < MAX_BULLETS*4; i++){
                        if(!mBullet[i]->Active()){
                            mBullet[i]->FireBullet(Pos());
                            mLastFiredSeries = currentTime;
                            break;
